...
import sys
sys.path.append(".")
from utils import gaussian, create_window
...
def msssim(img1, img2, window_size=11, size_average=True, val_range=2, normalize=True):
    ...
    # Normalize (to avoid NaNs during the first epoch of training unstable models, not compliant with original definition)
    if normalize:
        mssim = (mssim + 1) / 2
        mcs = (mcs + 1) / 2
